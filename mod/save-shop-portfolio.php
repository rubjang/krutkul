...
$ck=mysql_num_rows(mysql_query("select * from portfolio where id_member='$id_member' and id_shop='$id_shop'"));
if($ck<10){
$ok=mysql_query("Insert into portfolio values('','$id_member','$id_shop','".addslashes($category)."')");
if($ok){
...
echo"<script>alert('ไม่สามารถเพิ่มข้อมูลได้ เต็ม!!');window.parent.SentEr();</script>";
exit();
}
?>
